...
UNIFIED_SCRIPT="4.gen&judge&refine.py"
...
########################################
# Step 1: 患者表現 (mode=patient) LoRAなし 2GPU 分散
########################################

echo "=== [Patient] No LoRA 2GPU 分散生成 & Judge/Refine 開始 ==="
...
(
  ...
  for SID in "${GPU0_SHARDS[@]}"; do
    echo "[Patient][NoLoRA][GPU${GPU0}] shard ${SID}/${NUM_SHARDS_TOTAL} 実行中..."
    python "${UNIFIED_SCRIPT}" \
      --mode patient \
      --hpo-csv "${DATA_DIR}/HPO_symptom_depth_leq3_self_reportable_with_jp.csv" \
      --output "${DATA_DIR}/notLoRA/HPO_symptom_patient_expression_judge_refine.eques.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
      --gen-model "${GEN_MODEL}" \
      ...
      --shard-id "${SID}"\
      --wandb \
      --wandb-project "HPO_patient" \
      --wandb-tags "patient,unified,no-lora" \
      --log-each-round
  done
) &
PID_PAT_GPU0=$!
...
(
  ...
  for SID in "${GPU1_SHARDS[@]}"; do
    echo "[Patient][NoLoRA][GPU${GPU1}] shard ${SID}/${NUM_SHARDS_TOTAL} 実行中..."
    python "${UNIFIED_SCRIPT}" \
      --mode patient \
      --hpo-csv "${DATA_DIR}/HPO_symptom_depth_leq3_self_reportable_with_jp.csv" \
      --output "${DATA_DIR}/notLoRA/HPO_symptom_patient_expression_judge_refine.eques.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
...
echo "=== [Patient] No LoRA 全 shard 完了。マージします… ==="

OUT_PAT_NOTLORA="${DATA_DIR}/notLoRA/HPO_symptom_patient_expression_judge_refine.eques.jsonl"
: > "${OUT_PAT_NOTLORA}"
for SID in $(seq 0 $((NUM_SHARDS_TOTAL-1))); do
  cat "${DATA_DIR}/notLoRA/HPO_symptom_patient_expression_judge_refine.eques.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
    >> "${OUT_PAT_NOTLORA}"
done

echo "=== [Patient] No LoRA マージ完了: ${OUT_PAT_NOTLORA} ==="
echo
...
(
  ...
  for SID in "${GPU1_SHARDS[@]}"; do
    ...
    python "${UNIFIED_SCRIPT}" \
      ...
      --shard-id "${SID}"\
      --wandb \
      --wandb-project "HPO_unified_patternB" \
      --wandb-tags "patient,unified" \
      --log-each-round
  done
) &
PID_DOC_GPU1=$!
...
: > "${OUT_DOC_NOTLORA}"
for SID in $(seq 0 $((NUM_SHARDS_TOTAL-1))); do
  cat "${DATA_DIR}/notLoRA/HPO_doctor_expression_judge_refine.eques.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
    >> "${OUT_DOC_NOTLORA}"
done
...
(
  ...
  for SID in "${GPU0_SHARDS[@]}"; do
    ...
    python "${UNIFIED_SCRIPT}" \
      --mode patient \
      --hpo-csv "${DATA_DIR}/HPO_symptom_depth_leq3_self_reportable_with_jp.csv" \
      --output "${DATA_DIR}/HPO_symptom_patient_expression_judge_refine.eques_lora.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
      --gen-model "${GEN_MODEL}" \
...
OUT_PAT_LORA="${DATA_DIR}/HPO_symptom_patient_expression_judge_refine.eques_lora.jsonl"
: > "${OUT_PAT_LORA}"
for SID in $(seq 0 $((NUM_SHARDS_TOTAL-1))); do
  cat "${DATA_DIR}/HPO_symptom_patient_expression_judge_refine.eques_lora.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
    >> "${OUT_PAT_LORA}"
done
...
(
  ...
  for SID in "${GPU0_SHARDS[@]}"; do
    ...
    python "${UNIFIED_SCRIPT}" \
      --mode doctor \
      --hpo-csv "${DATA_DIR}/HPO_symptom_depth_leq3_with_jp.csv" \
      --output "${DATA_DIR}/HPO_doctor_expression_judge_refine.eques_lora.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
      --gen-model "${GEN_MODEL}" \
...
OUT_DOC_LORA="${DATA_DIR}/HPO_doctor_expression_judge_refine.eques_lora.jsonl"
: > "${OUT_DOC_LORA}"
for SID in $(seq 0 $((NUM_SHARDS_TOTAL-1))); do
  cat "${DATA_DIR}/HPO_doctor_expression_judge_refine.eques_lora.shard${SID}_of${NUM_SHARDS_TOTAL}.jsonl" \
    >> "${OUT_DOC_LORA}"
done
